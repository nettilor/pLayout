...
| All factors draws on Overview's neutral tile: no active-factor flood; the active line's band is tinted with its value's colour (0.42 alpha), rails are one-width colour blocks (rounded rects, `max(3.5, min(9, body × 0.14))`, radius `min(2, 0.3 × w)`) rather than capsules, and there is no solid marker | | | ⏳ | — | branch `all-factors-strips` (Sep 23 2026), not merged; the *Active marker* setting is inert under it |
