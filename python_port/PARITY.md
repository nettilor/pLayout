...
| *Colour blocks* settings (⌘, → Wells): the block's length and height as two multipliers on the rail formula — `stackBlockWidthScale` 0.5–2 and `stackBlockHeightScale` 0.4–1.2, default 1, clamped on load. Length is capped at `max(base, 0.35 × body)` so it never eats the name and never shrinks a tiny well's block; height is `0.82 × line × scale`, centred on the line; radius and the headline's wall come off the shorter side; the fit measures the name against the scaled slot | | | ⏳ | — | post-1.5.2, unreleased. Follows the rail row above |
